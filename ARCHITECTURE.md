...
### 7.4 Мобильные вкладки исполнителя (v2.2.0 — v2.2.2)

На экранах **&lt; 768px** для роли **исполнитель** на странице `/tasks` список заявок разбит на вкладки в **нижней навигации** (`mobile-nav`):

| Вкладка | Статусы в БД |
|---------|----------------|
| Новые | `new` |
| В работе | `in_progress`, `overdue`, `no_access` |
...
- **v2.2.2:** вкладки встроены в `mobile-nav` (класс `.executor-status-nav`, сетка **3×2**); отдельная панель `.executor-mobile-dock` удалена.
- **v2.2.2:** состояние передаётся через `ExecutorTasksNavContext` (`Tasks.jsx` → `Layout.jsx` → `ExecutorStatusNav`).
- Блок «Фильтры» для исполнителя **доступен** (сворачиваемый); выпадающий фильтр «Статус» скрыт при мобильных вкладках.
- Ссылка «← Дашборд» в шапке заявок — переход на другие разделы, когда `mobile-nav` занята вкладками.
...
Константа `EXECUTOR_MOBILE_TABS` и хелперы — в `client/src/utils.js`, контекст — `client/src/context/ExecutorTasksNavContext.jsx`, UI — `client/src/components/ExecutorStatusNav.jsx`.

### 7.5 Фильтры заявок (десктоп, v2.2.2)
