...
│   │   ├── angleDetector.js    # Вид съёмки: сбоку / спереди / сверху
│   │   ├── cleanlinessDetector.js # Чистота: пыль / грязь / мусор
│   │   ├── embedding.js        # Визуальный энкодер CLIP → вектор 512
│   │   ├── training.js         # Обучение на разметке bizadmin (nearest-centroid)
...
| `cv_settings` | Параметры CV (вкл/выкл, порог, запас, камера mobile, `cv_roles`, разрешение/качество фото исполнителя, проверки ракурса и чистоты, фото «до уборки») — bizadmin |
| `cv_training_samples` | Разметка фото из заявок для обучения CV-моделей — bizadmin |
| `cv_models` | Обученные классификаторы поверх признаков CLIP (прототипы классов, точность) |
...
| `GET /api/atms` | ✓ | ✓ | ✓ | ✓ | Список банкоматов; `search`, `limit`, `offset` → `{ items, total }`, без `limit` — массив |
| `POST /api/atms` | ✓ | ✓ | ✓ | — | Добавление банкомата |
| `GET /api/users` | все | все | только cleaner | — | Список пользователей; `search`, `limit`, `offset` → `{ items, total }`, без `limit` — массив |
...
| `GET /api/tasks?limit=&offset=` | ✓ | ✓ | ✓ | свои | Страница заявок `{ items, total }`; без `limit` — массив |
| `GET /api/tasks/status-counts` | ✓ | ✓ | ✓ | свои | Количество заявок по статусам для вкладок исполнителя |
| `GET /api/settings/cv/status` | ✓ | ✓ | ✓ | ✓ | Статус CV вкл/выкл (для UI) |
| `GET/POST /api/cv/training/*` | ✓ | — | — | — | Разметка фото и обучение CV-моделей (bizadmin) |
...
| Вид съёмки | `angleDetector.js` | `cv_angle`, `cv_angle_confidence`, `cv_angle_match` | Только при `angle_block_on_mismatch` |
...
#### Фото «до уборки» (v2.9.0)

Тип `before_top` — необязательный снимок сверху до начала работ. Для него запускается **только оценка чистоты**: банкомат не ищется (`cv_detected` остаётся `NULL`), вид съёмки не сверяется. Замечания попадают в отдельное поле `warnings.before_cleanliness` и **никогда не влияют на `ok`** — даже при включённом `cleanliness_block_on_dirty`. Обязательные для закрытия ракурсы перечислены в `REQUIRED_PHOTO_TYPES`, полный список принимаемых типов — в `ALL_PHOTO_TYPES` (`server/db.js`).

Оценки считаются как **суммы вероятностей по группам промптов** (`sumScore`), а не как максимум одного промпта: softmax размазан по всем меткам, и максимум систематически недобирает до порога. Softmax по подмножеству равен перенормировке сумм этого подмножества — это позволяет получить и распределение по классам, и попарные вероятности из одного прогона.

**Вид съёмки** — три класса `side` / `front` / `top`, по три промпта на класс (наборы сбалансированы, иначе сумма смещается к более крупной группе). Заявленные `left` и `right` оба ожидают вид `side`.

> **Лево и право не различаются.** На зеркальных ракурсах CLIP уверенно выдаёт один и тот же ответ, причём отрыв не коррелирует с правильностью. Различение сторон удалено; перепутанные местами «слева» и «справа» проверка не поймает.

**Чистота** — попарный контраст «замечание против чистого» для каждой из категорий `dust`, `dirt`, `trash`. Пыль и грязь сравниваются с чистым корпусом, мусор — с чистым полом; промпты про мусор не упоминают банкомат, иначе CLIP матчит сам терминал. В `cv_issues` попадают замечания выше `cleanliness_threshold`, итоговый уровень — самое серьёзное из найденного.

`getPhotoWarnings(photos)` собирает замечания и признаки блокировки; `getPhotoCvStatus()` учитывает их в `ok`.

**Калибровка (v2.8.1, 3 реальных фото):** верный вид набирает 0.55–0.71 при равновероятном исходе 0.33 — порог 0.45; 12 из 12 проверок вида верны. Чувствительность оценки чистоты не измерена: все доступные снимки — чистые банкоматы, пропуски возможны (мусорный пакет набрал 0.229 при пороге 0.65).

Проверка **обязательна для роли `executor`/`cleaner`** и **только если CV включена**.

#### Обучение на своих фото (v2.9.0)

Веса CLIP не дообучаются. `embedding.js` кодирует снимок в 512-мерный вектор через `CLIPVisionModelWithProjection`, `training.js` строит поверх этих признаков **классификатор по ближайшему центроиду**: для каждого класса усредняются нормированные векторы примеров, новое фото относится к ближайшему прототипу по косинусной близости с softmax по температуре. Дообучение самого CLIP потребовало бы тысяч размеченных фото и GPU; прототипы дают осмысленный результат от пары десятков примеров и обучаются за секунды.

| Сущность | Где | Назначение |
|----------|-----|------------|
| Разметка | `cv_training_samples` | Фото, тип модели (`cleanliness` / `view`), метка, кэш эмбеддинга |
| Модель | `cv_models` | Прототипы классов (JSON), число примеров, точность, флаг `active` |
| API | `server/routes/cvTraining.js` | `/api/cv/training/*`, только bizadmin |
| UI | `client/src/components/CvTrainingPanel.jsx` | Настройки → «🎓 Обучение модели» |

Класс входит в модель только при наличии минимум двух примеров — один пример даёт неустойчивый прототип. Качество показывается как **точность leave-one-out**: каждый пример по очереди исключается из прототипов и классифицируется остальными.

Детекторы сначала спрашивают `getActiveModel(kind)`; если активной модели нет, работают по промптам. Эмбеддинг считается один раз в `analyzePhoto()` и переиспользуется обоими детекторами, а при отсутствии обученных моделей не считается вовсе. `POST /api/cv/training/reset` снимает флаг `active` — проверки возвращаются к zero-shot, разметка сохраняется.
...
|---------|:------------:|--------|
| Банкомат / ID УС | да | `US-001` (`serial_number`) |
...
- **v2.4.6:** цвет вкладки по статусу (CSS `--tab-accent`), иконка, полоска сверху, подсветка активной вкладки; пульсация счётчика на `overdue` и `emergency`.
...
- Карточки заявок вместо таблицы (`TaskCard.jsx`); **v2.5.1** — компактные карточки и таблица `tasks-table` (меньше отступы, `btn-xs`, обрезка длинных ячеек)
- **Плотность списка** (v2.9.1): тело карточки — одна цель нажатия (открывает заявку), в ряду одна главная кнопка по контексту, остальные в меню «…» (`.task-card-menu`); на странице заявок скрыта подпись, ужат заголовок, у свёрнутых фильтров и у обёртки списка убраны отступы карточки. Освобождается ~190 px первого экрана
- **Постраничная загрузка** (v2.9.1): 20 заявок на мобильных, 50 на десктопе, дальше «Показать ещё». Вкладки исполнителя фильтруют по статусу на сервере, счётчики — из `GET /api/tasks/status-counts`; офлайн кэш отдаётся целиком и фильтруется на клиенте
- **Реестры Устройства и Сотрудники** (v2.9.2): на экранах < 768px `EntityFieldTable` с `cards` рендерит те же колонки карточками (`.entity-cards`) — заголовок из первого поля, остальные строками «подпись — значение», действия в углу; вместо горизонтальной прокрутки таблицы. Строка поиска (задержка 300 мс) и постраничная загрузка 20/50 с «Показать ещё»
- **Определение мобильного экрана**: `useIsMobile()` (`src/hooks/useIsMobile.js`), граница 768px — та же, что в media-запросах
- **Страница входа** (v2.5.1): без блока демо-аккаунтов; учётные записи в БД не менялись
- **Сворачиваемые фильтры** (v2.1.0): на экранах &lt; 768px блок фильтров в `Tasks.jsx` по умолчанию скрыт; кнопка «Фильтры» с бейджем активных фильтров (для менеджеров и десктопа); фильтр **ID УС** (`serial_number`, v2.4.1)
- **Колонка ID УС** (v2.4.1): в десктопной таблице заявок, карточках (`TaskCard`), модалке (`TaskModal`) и экспорте Excel
- **Вкладки исполнителя** (v2.2.0+): на мобильных у исполнителя на странице заявок — вкладки по статусам со счётчиками в `mobile-nav` (сетка 3×2); фильтры остаются доступными. **v2.9.1** — вкладка передаётся в запрос как `status`, счётчики приходят отдельным запросом
...
|--------|------|-----------|
| v2.8.1 | 2026-09-06 | Калибровка проверок ракурса и чистоты на живых фото — см. [CHANGELOG.md](./CHANGELOG.md) |
