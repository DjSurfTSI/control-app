...
### 7.4 Мобильные вкладки исполнителя (v2.2.0, v2.2.1)

На экранах **&lt; 768px** для роли **исполнитель** список заявок разбит на вкладки:

| Вкладка | Короткая подпись (v2.2.1) | Статусы в БД |
|---------|---------------------------|----------------|
| Новые | Новые | `new` |
| В работе | В работе | `in_progress`, `overdue`, `no_access` |
| Завершенные | Готовые | `completed` |
| Отмененные | Отмена | `cancelled` |
| Возврат | Возврат | `returned` |
| Экстренно | Срочно | `emergency` |

- Каждая вкладка показывает **счётчик** заявок (бейдж).
- **v2.2.1:** панель вкладок фиксирована **внизу экрана** (`.executor-mobile-dock`, над `mobile-nav`), сетка **3×2** — все вкладки видны без прокрутки.
- **v2.2.1:** блок «Фильтры» для исполнителя **восстановлен** (сворачиваемый); выпадающий фильтр «Статус» скрыт при активных мобильных вкладках.
- Загрузка с API **без** фильтра `status` (все заявки исполнителя), фильтрация на клиенте.
- `?status=overdue` в URL открывает вкладку «В работе».

Константа `EXECUTOR_MOBILE_TABS` (поле `shortLabel`) и хелперы — в `client/src/utils.js`, компонент `ExecutorMobileTabs` — в `Tasks.jsx`, стили дока — `client/src/index.css`.
...
- **Вкладки исполнителя** (v2.2.0, v2.2.1): на мобильных у исполнителя — панель вкладок по статусам со счётчиками внизу экрана (сетка 3×2); фильтры остаются доступными
