...
router.post('/:taskId', upload.single('photo'), async (req, res) => {
  ...
  const cvResult = await validatePhoto(req.file.path, photo.id);

  const updated = db.prepare('SELECT * FROM task_photos WHERE id = ?').get(photo.id);

  res.status(201).json({
    ...mapPhoto(req, req.params.taskId, updated),
    cv_warning: !cvResult.detected && !cvResult.skipped
      ? 'Банкомат не обнаружен на фото. Переснимите ракурс.'
      : null,
  });
});
